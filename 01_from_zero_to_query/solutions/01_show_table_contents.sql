-- 01.01 first get a list of tables with
.tables


-- 01.02 then explore all columns of the category table, using
SELECT * FROM category;

-- 01.03 some tables have many many records. let's LIMIT the output to 5 records:
SELECT * FROM actor LIMIT 5;

-- 01.03 explore the first few records of a table:
...
-- you can also select specific columns, separated with a comma:
SELECT title, rating, length, release_year FROM film LIMIT 10;
SELECT first_name, last_name FROM staff;

-- 01.04 you can rename the columns on output with: SELECT col AS colname FROM table:
